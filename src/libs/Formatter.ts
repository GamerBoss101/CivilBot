...
export default class Formatter {
    ...
    parseString(input: any) {
        ...
        input.split(' ').forEach(async(word: any) => {
            ...
            if(key[0] == 'bot') result = result.replace(`{${key[0]}_${key[1]}}`, this.getConfig()[key[0]][key[1]]);
            ...
            if(key[0] == 'client') {
                result = result.replace(`{${key.join("_")}}`, this.jsonPathToValue(this.getConfig(), key.slice(1).join(".")));
            }
            if(this.obj != null) {
